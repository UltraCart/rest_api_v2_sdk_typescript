...
import {
    ...
    CustomerEmailSuppressionRequestFromJSON,
    CustomerEmailSuppressionRequestToJSON,
...
export class CustomerApi extends runtime.BaseAPI implements CustomerApiInterface {
     ...
     * Clear email suppression for a customer
     */
    async updateCustomerEmailSuppressionRaw(requestParameters: UpdateCustomerEmailSuppressionRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CustomerEmailSuppressionResponse>> {
        if (requestParameters.customerProfileOid === null || requestParameters.customerProfileOid === undefined) {
        ...
        const response = await this.request({
            path: `/customer/customers/{customer_profile_oid}/email_suppression`.replace(`{${"customer_profile_oid"}}`, encodeURIComponent(String(requestParameters.customerProfileOid))),
            method: 'POST',
            headers: headerParameters,
            query: queryParameters,
            body: CustomerEmailSuppressionRequestToJSON(requestParameters.suppressionChanges),
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => CustomerEmailSuppressionResponseFromJSON(jsonValue));
    }
     ...
     * Clear email suppression for a customer
     */
    async updateCustomerEmailSuppression(requestParameters: UpdateCustomerEmailSuppressionRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<CustomerEmailSuppressionResponse> {
        const response = await this.updateCustomerEmailSuppressionRaw(requestParameters, initOverrides);
        return await response.value();
    }
